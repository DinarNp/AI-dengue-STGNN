...
Provides CRUD operations and data validation, keyed by epidemiological week.
"""
import pandas as pd
import os
...
def _weeks_in_year(year: int) -> int:
    """Number of ISO epi-weeks in a given year (52 or 53)."""
    return Week.fromdate(date(year, 12, 28), system='iso').week
...
class CSVDataManager:
    ...
    def export_dengue_cases(self, filename='dengue_cases.csv') -> Dict:
        ...
        try:
            cases = DengueCase.query.order_by(
                DengueCase.epi_year, DengueCase.epi_week, DengueCase.regency_id
            ).all()

            records = []
            for case in cases:
                records.append({
                    'Epi_Year': case.epi_year,
                    'Epi_Week': case.epi_week,
    ...
    def export_climate_data(self, filename='climate_data.csv') -> Dict:
        ...
        try:
            climate = ClimateData.query.order_by(
                ClimateData.epi_year, ClimateData.epi_week, ClimateData.regency_id
            ).all()
    ...
    def export_ndvi_data(self, filename='ndvi_data.csv') -> Dict:
        ...
        try:
            ndvi = NDVIData.query.order_by(
                NDVIData.epi_year, NDVIData.epi_week, NDVIData.regency_id
            ).all()
    ...
    def import_dengue_cases(self, csv_path: str, user_id: int) -> Dict:
        ...
        try:
            ...
            required = ['Epi_Year', 'Epi_Week', 'Region', 'Cases']
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    try:
                        ...
                        errors.append(f"Row {idx+1}: Invalid epi-week {epi_week} for year {epi_year}")
                        continue
    ...
    def import_climate_data(self, csv_path: str) -> Dict:
        ...
        try:
            ...
            required = ['Epi_Year', 'Epi_Week', 'Region']
            if not all(col in df.columns for col in required):
                return {
                    'success': False,
                    'message': f'Missing required columns. Need: {required}'
    ...
    def import_ndvi_data(self, csv_path: str) -> Dict:
        ...
        try:
            ...
            required = ['Epi_Year', 'Epi_Week', 'Region', 'NDVI']
            if not all(col in df.columns for col in required):
                return {
                    'success': False,
                    'message': f'Missing required columns. Need: {required}'
    ...
    def merge_data_for_training(self, year_start: int, year_end: int,
        ...
        Creates a combined CSV file with all features, one row per
        (epi_year, epi_week, regency).
        """
        try:
            records = []
            ...
            for year in range(year_start, year_end + 1):
                for epi_week in range(1, _weeks_in_year(year) + 1):
                    for regency in regencies:
                        # Get all data
                        dengue = DengueCase.query.filter_by(
                            regency_id=regency.id, epi_year=year, epi_week=epi_week
                        ).first()
                        ...
                        # Create merged record (column names match the canonical
                        # revision_experiments training CSV: Year,Region,Week,Cases,...)
                        record = {
                            'Year': year,
                            'Region': regency.name,
                            'Week': epi_week,
            ...
            total_expected = len(regencies) * sum(
                _weeks_in_year(y) for y in range(year_start, year_end + 1)
            )
            completeness = len(records) / total_expected * 100 if total_expected else 0
    ...
    def validate_data_completeness(self, year_start: int, year_end: int) -> Dict:
        ...
        try:
            regencies = Regency.query.filter_by(is_active=True).all()
            weeks_expected = sum(_weeks_in_year(y) for y in range(year_start, year_end + 1))
            total_expected = len(regencies) * weeks_expected

            # Count available records
            dengue_count = DengueCase.query.filter(
                DengueCase.epi_year >= year_start,
                DengueCase.epi_year <= year_end
            ).count()

            climate_count = ClimateData.query.filter(
                ClimateData.epi_year >= year_start,
                ClimateData.epi_year <= year_end
            ).count()

            ndvi_count = NDVIData.query.filter(
                NDVIData.epi_year >= year_start,
                NDVIData.epi_year <= year_end
            ).count()
