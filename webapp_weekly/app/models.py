...
class DengueCase(db.Model):
    """Dengue case data model, keyed by epidemiological week (not calendar month)."""
    ...
    epi_week = db.Column(db.Integer, nullable=False)  # ISO/epi week, 1-53
    month = db.Column(db.Integer, nullable=True)  # derived, for display only — not part of the unique key
    ...
    # Unique constraint for regency-epi_year-epi_week combination
    __table_args__ = (
        db.UniqueConstraint('regency_id', 'epi_year', 'epi_week', name='unique_regency_epiyear_epiweek'),
    )

    def __repr__(self):
        return f'<DengueCase {self.regency.name} {self.epi_year}-W{self.epi_week}: {self.cases}>'


class ClimateData(db.Model):
    """Climate/weather data model, keyed by epidemiological week."""
    ...
    month = db.Column(db.Integer, nullable=True)  # derived, for display only
    ...
    # Unique constraint
    __table_args__ = (
        db.UniqueConstraint('regency_id', 'epi_year', 'epi_week', name='unique_climate_regency_epiyear_epiweek'),
    )

    def __repr__(self):
        return f'<ClimateData {self.regency.name} {self.epi_year}-W{self.epi_week}>'


class NDVIData(db.Model):
    """NDVI (Normalized Difference Vegetation Index) data model, keyed by epidemiological week."""
    ...
    data_source = db.Column(db.String(50), default='modis_neo_csv')  # 'modis_neo_csv', 'manual'
    processing_date = db.Column(db.DateTime, default=datetime.utcnow)
    is_imputed = db.Column(db.Boolean, default=False)  # True if value was carried forward from the last known 16-day MODIS reading

    # Unique constraint
    __table_args__ = (
        db.UniqueConstraint('regency_id', 'epi_year', 'epi_week', name='unique_ndvi_regency_epiyear_epiweek'),
    )

    def __repr__(self):
        return f'<NDVIData {self.regency.name} {self.epi_year}-W{self.epi_week}: {self.ndvi_value}>'


class Prediction(db.Model):
    """Dengue prediction model results, keyed by epidemiological week."""
    ...
    actual_cases = db.Column(db.Integer)  # Filled in after the week's case data is reported

    model_version = db.Column(db.String(50))
    prediction_date = db.Column(db.DateTime, default=datetime.utcnow)

    # Risk classification — statistical "endemic channel" method (mean + 1.25*SD of
    # this region's cases in the same epi-week across prior years), matching
    # webapp_restructured/app/services/prediction.py::_calculate_risk_level.
    # Binary only: 'alert' / 'no_alert'.
    risk_level = db.Column(db.String(20))       # 'alert', 'no_alert'
    alert_threshold = db.Column(db.Float)       # mean + 1.25*SD threshold used
    hist_mean = db.Column(db.Float)             # historical mean for this regency+epi-week
    hist_sd = db.Column(db.Float)               # historical SD for this regency+epi-week
    exceed_probability = db.Column(db.Float)    # P(actual > threshold | actual ~ N(predicted, hist_sd)) %

    def __repr__(self):
        return f'<Prediction {self.regency.name} {self.epi_year}-W{self.epi_week}: {self.predicted_cases:.1f}>'
