"""
APP-MONTH.PY - Modified version of app3.py for MONTHLY dengue data prediction
This version is adapted to work with monthly_data_3kab.csv dataset

Key modifications from app3.py:
1. Data source: Uses monthly_data_3kab.csv instead of fix.csv
2. Temporal features: Uses Month column instead of Week column
3. Date creation: Creates dates from Year-Month instead of Year-Week
4. Lag features: Uses monthly lags (1m, 2m, 3m, 6m) instead of weekly lags (1w, 2w, 4w, 8w)
5. Cyclical encoding: Focuses on month and quarter instead of week_of_year and day_of_year
6. Sample data: Generates 48 months (4 years) instead of 208 weeks
7. Aggregation period: All statistics and predictions are at monthly granularity
"""
...
class SpatioTemporalDataSplitter:
    ...
    def create_temporal_features(self):
        ...
        if self.temporal_col not in self.data.columns:
            ...
            if 'Year' in self.data.columns and 'Month' in self.data.columns:
                ...
                raise ValueError("No temporal information available (need Year and Month columns)")
        ...
        # MODIFIED FOR MONTHLY DATA: Focus on month-level cyclical features
        for col, max_val in [('month', 12), ('quarter', 4)]:
    ...
    def create_lag_features(self, lag_months=[1, 2, 3, 6]):
        """Create lagged features for environmental and case variables"""
        # MODIFIED FOR MONTHLY DATA: Use months instead of weeks
        print(f"Creating lag features for {lag_months} months...")
        ...
        for location_coords in self.data[self.spatial_cols].drop_duplicates().values:
            ...
            # MODIFIED FOR MONTHLY DATA: Use 'm' suffix for months instead of 'w' for weeks
            for lag in lag_months:
                # Lag environmental variables
                for var in available_env_vars:
                    lag_col = f'{var}_lag_{lag}m'
                ...
                if self.target_col in location_data.columns:
                    lag_cases_col = f'cases_lag_{lag}m'
                    lagged_cases = location_data[self.target_col].shift(lag)
                    self.data.loc[location_indices, lag_cases_col] = lagged_cases
        
        print(f"Created {len(lag_months)} lag periods for {len(available_env_vars)} environmental variables")
    ...
    def prepare_sequences(self, data, sequence_length=8, forecast_horizon=1):
        ...
        # MODIFIED FOR MONTHLY DATA: Exclude 'Month' instead of 'Week'
        exclude_cols = [self.temporal_col, self.target_col, 'Year', 'Month', 'Week', 'Region'] + self.spatial_cols
    ...
    def prepare_data_for_dataset(df, target_col='cases', location_col='Regency'):
        ...
        exclude_cols = [
            target_col, location_col, 'date', 'Year', 'Month', 'Week', 
...
def create_sample_data():
    ...
    # MODIFIED FOR MONTHLY DATA: Create monthly instead of weekly sample data
    np.random.seed(42)  # For reproducible results
    
    # Create sample data
    n_months = 48  # 4 years of monthly data (12 months * 4 years)
    locations = [
    ...
    # MODIFIED FOR MONTHLY DATA: Iterate over months instead of weeks
    for month in range(n_months):
        ...
        for loc in locations:
            ...
            seasonal_temp = 27 + 3 * np.sin(2 * np.pi * month_of_year / 12)
            seasonal_precip = 75 + 50 * np.sin(2 * np.pi * month_of_year / 12 + np.pi/4)
            seasonal_humidity = 75 + 10 * np.sin(2 * np.pi * month_of_year / 12)
            ...
            data.append({
                ...
                'Month': current_date.month,  # Use Month instead of Week
                'Region': loc['name'],
                'latitude': loc['lat'],
                'longitude': loc['lon'],
                'cases': max(0, np.random.poisson(5) + int(np.random.normal(0, 3))),  # Higher for monthly aggregation
                ...
                'ndvi': np.clip(0.4 + 0.2*np.sin(2 * np.pi * month_of_year / 12) + np.random.normal(0, 0.1), 0.1, 0.9)
            })
...
def load_data():
    ...
    try:
        ...
        # MODIFIED FOR MONTHLY DATA: Use monthly_data_3kab.csv instead of fix.csv
        data_path = "data/monthly_data_3kab.csv"
        ...
        # MODIFIED FOR MONTHLY DATA: Sequence length represents months (e.g., 12 = 1 year, 6 = 6 months)
        split_results = splitter.run_complete_split(create_sequences=False, sequence_length=6, split_method='stratified')
    ...
    try:
        ...
        if AI_MODULES_AVAILABLE:
            ...
            try:
                ...
                # MODIFIED FOR MONTHLY DATA: Use monthly_data_3kab.csv
                model_instance, metrics, metadata, history = system.run_complete_pipeline("data/monthly_data_3kab.csv", generate_paper_analysis=True )
...
def plot_data(regency_name):
    ...
    try:
        ...
        # MODIFIED FOR MONTHLY DATA: Use month lags instead of week lags
        for lag_col in ['cases_lag_3m', 'temperature_avg_lag_3m', 'cases_lag_6m']:
        ...
        for regency in regency_list:
            ...
            if not regency_data.empty:
                ...
                if processed_data is not None:
                    ...
                    # Add lag information
                    # MODIFIED FOR MONTHLY DATA: Use month lags instead of week lags
                    for lag_var in ['cases_lag_3m', 'temperature_avg_lag_3m', 'cases_lag_6m']:
                        if lag_var in regency_data.columns:
                            lag_val = regency_data[lag_var].iloc[-1] if len(regency_data) > 0 and not pd.isna(regency_data[lag_var].iloc[-1]) else 0.0
                            risk_entry[lag_var] = float(lag_val)
...
if __name__ == '__main__':
    print("="*70)
    print("Starting ExplainDengue Flask Application - MONTHLY DATA VERSION")
    print("="*70)
    print(f"Dataset: monthly_data_3kab.csv (Monthly aggregated dengue data)")
    print(f"AI Modules Available: {AI_MODULES_AVAILABLE}")
    print(f"Temporal Granularity: MONTHLY (instead of weekly)")
    print(f"Lag Features: 1, 2, 3, 6 months (instead of 1, 2, 4, 8 weeks)")
    print("="*70)
